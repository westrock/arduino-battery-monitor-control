...
/*
  ...
  Modified by:     Ed Figarsky <efigarsky@gmail.com>
  Available from:  https://github.com/rodan/ds3231
...
#define IS_LEAP_YEAR(y) (isLeapYear(y))
...
void DS3231_set(DateTimeDS3231 t)
{
    uint8_t i, century;
...
void DS3231_get(DateTimeDS3231 *t)
{
...
uint32_t get_unixtime(DateTimeDS3231 t)
{
    uint8_t i;
    uint16_t d;
    int16_t y;
...
void DDDtoMMDD(DateTimeDS3231* dateTime, uint8_t* mm, uint8_t* dd)
{
	...
	while ((dateTime->yday > monthDays[i]) && (i < 11))
	{
		i++;
	}
	*dd = dateTime->yday - monthDays[i - 1];
	*mm = i;
}
